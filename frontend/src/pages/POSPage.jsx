...
function POSPage() {
  ...
  const products = useCartStore((state) => state.products); // for useMemo deps

  // ─── fetch catalogue from backend ────────────────────────────────────────
      ...
      <div className="flex h-screen flex-col overflow-hidden bg-[#fef9f2] text-[#3d0c02]">
        <header className="flex h-[56px] shrink-0 items-center justify-between bg-[#3d0c02] px-6 text-white">
          <div className="flex items-center gap-4">
            ...
            <span className="border-l border-white/20 pl-4 text-sm opacity-60">Oct 24, 2023</span>
                ...
                visibleProducts.map((product) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="mt-2 font-bold text-[#E8A020]">₹{product.price}</p>
                    </div>
                  </button>
                ))}
            </div>
            ...
            <div className="border-t border-[#ded9d3] bg-white p-6 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
              <div className="mb-6 space-y-3">
                ...
                <div className="flex items-center justify-between border-t border-dashed border-[#ded9d3] pt-4">
                  ...
                  <span className="text-4xl font-extrabold text-[#3d0c02]">₹{total}</span>
                </div>
              </div>
            ...
            <div className="w-full max-w-[440px] rounded-2xl border border-[#ded9d3] bg-[#fef9f2] shadow-2xl">
              <div className="flex flex-col gap-6 p-6">
                <div>
                  ...
                  <p className="mt-1 text-sm text-[#54433f]">Please review the daily summary before closing.</p>
                </div>
                <div className="space-y-3 rounded-xl border border-[#ded9d3]/50 bg-[#f8f3ec] p-4">
                  <div className="flex justify-between"><span>Pending Orders</span><span className="font-bold">3 Orders</span></div>
                  <div className="flex justify-between"><span>Today's Sales</span><span className="font-bold">₹28,450</span></div>
                  <div className="flex justify-between"><span>Cash in Drawer</span><span className="font-bold">₹12,420</span></div>
                </div>
                <div className="flex gap-3">
                  <button onClick={() => setShowCloseSaleModal(false)} className="h-14 flex-1 rounded-xl border-2 border-[#ded9d3] font-bold text-[#3d0c02]">Cancel</button>
                  <button onClick={() => setShowCloseSaleModal(false)} className="h-14 flex-1 rounded-xl bg-red-600 text-lg font-extrabold text-white shadow-lg">Confirm & Close Sale</button>
                </div>
              </div>
            </div>
          </div>
